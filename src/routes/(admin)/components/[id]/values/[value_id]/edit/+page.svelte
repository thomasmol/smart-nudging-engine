...
<div class="container">
	<div class="mb-4 flex justify-between">
		<div>
			<h1 class="mb-2 text-xl font-semibold text-slate-800">Update Component Value</h1>
			<h2 class="mb-2 text-base text-slate-800">Of Component Type {data.componentValue.ComponentType.label}</h2>
		</div>
	</div>

	<div class="relative overflow-x-auto rounded-lg border bg-white p-5">
		<form method="post" action="/components/{data.componentValue.ComponentType.id}/values/{data.componentValue.id}?/update">
			<div class="mb-4 grid gap-4 sm:mb-5 sm:grid-cols-2 sm:gap-6">
				<div class="w-full">
					<label for="value" class="mb-2 block text-sm font-medium text-gray-900">Value</label>
					<input
						type="text"
						name="value"
						id="value"
						value="{data.componentValue.value}"
			...
			<div class="flex items-center space-x-4">
				...
				<button
					formaction="/components/{data.componentValue.ComponentType.id}/values/{data.componentValue.id}?/destroy"
