...
<div class="container">
	<div class="mb-4 flex justify-between">
		<div>
			<h1 class="mb-2 text-xl font-semibold text-slate-800">Update {data.nudgee.id}</h1>
		</div>
	</div>

	<div class="relative overflow-x-auto rounded-lg border bg-white p-5">
		<form method="post" action="/nudgees/{data.nudgee.id}?/update">
			<div class="mb-4 grid gap-4 sm:mb-5 sm:grid-cols-2 sm:gap-6">
				<div class="w-full">
					...
					<input
						...
						value={data.nudgee.id}
						placeholder="Some Gen4 UUID"
						required />
				</div>
				<div class="w-full">
					<label for="profile" class="mb-2 block text-sm font-medium text-gray-900">Type</label>
					<input
						type="text"
						name="profile"
						id="profile"
						class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-slate-600 focus:ring-slate-600"
						value={data.nudgee.profile}
			...
			<div class="flex items-center space-x-4">
				...
				<button
					formaction="/nudgees/{data.nudgee.id}?/destroy"
